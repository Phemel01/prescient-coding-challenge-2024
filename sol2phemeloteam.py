...
from sklearn.calibration import CalibratedClassifierCV  # To get probability output from SVC
...
start_test = datetime.date(2024, 1, 1)   # test set is 2024 data
...
df_signals.sort_values(by='date', inplace=True)  # sort by date for walk-forward validation

# Loop for walk-forward validation
for i in range(len(df_signals)):

    if verbose: 
        print('---> doing', df_signals.loc[i, 'date'])

    # This iteration's training set
    df_trainx = df_x[df_x['date'] < df_signals.loc[i, 'date']].copy()
    df_trainx.drop(labels=df_trainx[df_trainx['date'] == df_trainx['date'].max()].index, inplace=True)  # no overlap
    ...
    # Initialize and fit models (only for the first iteration)
    if i == 0:
        rf_model = RandomForestClassifier(n_estimators=50, max_depth=5, random_state=42)
        svc_model = CalibratedClassifierCV(LinearSVC(max_iter=1000, random_state=42))

    # Fit the models on the training data
    rf_model.fit(np.array(df_trainx[list_vars1]), df_trainy['label'].values)
    svc_model.fit(np.array(df_trainx[list_vars1]), df_trainy['label'].values)

    # Predict probabilities with both models
    rf_pred = rf_model.predict_proba(np.array(df_testx[list_vars1]))[:, 1]
    svc_pred = svc_model.predict_proba(np.array(df_testx[list_vars1]))[:, 1]

    # Combine predictions (average)
    combined_pred = (rf_pred + svc_pred) / 2

    # Generate signals
    df_testy['signal'] = combined_pred
    df_testy['pred'] = (combined_pred >= 0.5).astype(int)  # Threshold at 0.5 for buys
    ...
    # Add accuracy and signals to the signals dataframe
...
# Create the buy matrix for the top 10 signals
...
# Set 1 for the top 10 strongest signals
